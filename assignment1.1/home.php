<?php
/* php code can go anywhere but we will be starting almost all our documents
 * with php code There is no code here at the moment */
?>
<!DOCTYPE HTML>
<html lang="en">

    <head>
        <title>Making The World A Better Place</title>
        <meta charset="utf-8">
        <meta name="author" content="Nana Antwi">
        <meta name="description" content="mainindex">
        
         <link rel="stylesheet"
             href="style.css"
             type="text/css"
             media="screen">

        
    </head>

    <body>

        <h1>Nana Antwi</h1>
        <h2>Assignment 1.2</h2>
        <h3>Making The world A Better Place</h3>


        <nav>
            <a href="index.php"> Home</a>
            <a href="page1.php">Introduction</a>
            <a href="page2.php">Global Warming / Hunger</a>
            <a href="page3.php">Political Unrest / Natural Disaster / Economic Downturn</a>
             <a href="page4.php">Image Map of World Problems</a>
            <a href="specdu.pdf">Specification Document</a>

        </nav>


        <img src="pic.jpg" alt="My website logo">

           

    </body>

</html>
